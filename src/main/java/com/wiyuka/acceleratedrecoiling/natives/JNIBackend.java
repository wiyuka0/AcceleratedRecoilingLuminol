...
public class JNIBackend implements INativeBackend {
    ...
    // 引入 JUL Logger
    private static final Logger LOGGER = Logger.getLogger(JNIBackend.class.getName());
    ...
    private static class ThreadState {
        ...
        void destroy() {
            if (contextPtr != 0) {
                try { destroyCtx(contextPtr); } catch (Throwable e) { LOGGER.log(Level.SEVERE, "Failed to destroy ctx", e); }
                contextPtr = 0;
            }
            if (configPtr != 0) {
                try { destroyCfg(configPtr); } catch (Throwable e) { LOGGER.log(Level.SEVERE, "Failed to destroy cfg", e); }
                configPtr = 0;
            }
            bufA = null;
            bufB = null;
        ...
        for (ThreadState state : ALL_THREAD_STATES) {
            if (state.configPtr != 0) {
                try {
                    ...
                    LOGGER.log(Level.SEVERE, "Failed to update JNI native config", e);
                }
            }
        }
    }
    ...
    public void initialize() {
        ...
        try (InputStream dllStream = AcceleratedRecoiling.class.getResourceAsStream("/" + fullDllName)) {
            ...
            try (OutputStream out = new FileOutputStream(tempDll)) {
                ...
                // JUL 使用 {0}, {1} 进行参数格式化
                LOGGER.log(Level.INFO, "Extracted JNI native library to temp: {0}", dllPath);
            }
        } catch (IOException e) {
        ...
        // 注意：原代码的 ".jso n" 疑似拼写错误，若不需要空格可自行修改为 ".json"
        ...
        try {
            ...
            LOGGER.log(Level.WARNING, "Failed to read config, reason: {0}. Using default.", e.getMessage());
        ...
        try {
            ...
            LOGGER.log(Level.WARNING, "Config broken: {0}. Overwriting.", e.getMessage());
            foldConfig.deleteOnExit();
            createConfigFile(foldConfig, defaultConfig);
